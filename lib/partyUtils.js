...
// Non-political advertisers to exclude (government bodies, corporations, media houses, etc.)
const NON_POLITICAL_ADVERTISERS = [
  'central bureau of communication',
  'davp',
  'directorate of advertising and visual publicity',
  'andhra pradesh digital corporation',
  'digital corporation limited',
  't.v. today network',
  'tv today network',
  'jagran prakashan',
  'business standard'
...
const PARTY_KEYWORDS = {
  ...
  'AIADMK': [
    ...
    'edappadi',
    'makkalai kaapom',
    'tamizhagathai meetpom',
    'makkalai kaapom tamizhagathai meetpom',
    'eps',
  ...
  INC: [
    ...
    'pcc',
    'dpcc',
    'सरकार बदलो, बिहार बदलो',
    'sarkar badlo, bihar badlo',
    'sarkar badlo bihar badlo'
  ],
  AAP: [
    'aam aadmi party',
...
export function classifyParty(pageId, bylines) {
  ...
  // First check if it's a non-political advertiser (government bodies, corporations)
  if (bylines && isNonPoliticalAdvertiser(bylines)) {
    return 'Others';
  }

  const searchText = `${pageId || ''} ${bylines || ''}`.toLowerCase();

  // Check advertiser mappings (exact match for companies advertising for parties)
  if (bylines) {
...
/**
 * Check if an advertiser is non-political (government body, corporation, etc.)
 * @param {string} advertiserName - Advertiser/page name
 * @returns {boolean} - True if it's a non-political advertiser
 */
export function isNonPoliticalAdvertiser(advertiserName) {
  if (!advertiserName) return false;
  const nameLower = advertiserName.toLowerCase();
  return NON_POLITICAL_ADVERTISERS.some(excludedName =>
    nameLower.includes(excludedName)
  );
}
