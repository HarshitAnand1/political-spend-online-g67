...
This script collects ads for LOCAL PostgreSQL database (fast collection).
For syncing to RDS, use sync_to_rds.py after collection.

To use with Local PostgreSQL:
    - Ensure your environment variables are set for LOCAL_PG_USER, LOCAL_PG_PASSWORD, LOCAL_PG_HOST, LOCAL_PG_DATABASE, LOCAL_PG_PORT.
    - Or edit the defaults in push_to_local_db.py if needed.
    - The script will connect to local PostgreSQL by default.

Usage: python3 collect_local.py <country_code> --start-date YYYY-MM-DD --end-date YYYY-MM-DD

Example: python3 collect_local.py IN --start-date 2025-11-02 --end-date 2025-11-02
...
from push_to_local_db import SQLInserter  # Using the LOCAL database module
